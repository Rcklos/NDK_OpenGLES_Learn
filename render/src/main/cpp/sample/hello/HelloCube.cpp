...
static GLfloat vertices[4 * HelloCube::FACE_SIZE * HelloCube::VERTEX_POS_SIZE] = {
        // 前面正视
        CUBE_FRONT_LB,      // 左下
        CUBE_FRONT_RB,      // 右下
        CUBE_FRONT_RT,      // 右上
...
static GLfloat texes[4 * HelloCube::FACE_SIZE * HelloCube::VERTEX_TEX_SIZE] = {
        // 前面
        0, 0,
        1,0,
        1, 1,
...
static GLint vertexStride = (HelloCube::VERTEX_POS_SIZE) * sizeof(GLfloat);

static GLushort indices[HelloCube::INDICES_SIZE];

void HelloCube::Create() {
    // 加载shader源码
    VERTEX_SHADER = GLUtils::openTextFile("shader/vertex/hello_cube.glsl");
...
GLuint HelloCube::CreateRectangleTexture2D() {
...
float HelloCube::computeDeltaTime() {
    if (mLastTime == 0) {
...
void HelloCube::update(float deltaTime) {
