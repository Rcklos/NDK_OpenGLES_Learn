...
void SimpleScale::update(float deltaTime) {
    if(scale > 0.1f) scale -= 0.1f * deltaTime;
    else scale = 1.0f;

    trans = glm::scale(glm::mat4(1.0f), glm::vec3(scale, scale, scale));
...
float SimpleScale::computeDeltaTime() {
    if (mLastTime == 0) {
